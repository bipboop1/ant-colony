...
class Nest:
    ...
    def spawn_ant(self, ants):
        if self.food_deposited >= FOOD_TO_SPAWN_ANT:
            ants.append(Ant(self.x, self.y, self))  # Spawn a new ant at the nest
...
def draw_counters(screen, ants, total_food_collected, total_ants_spawned, time_elapsed):
...
def main():
    ...
    ants = [Ant(nest.x, nest.y, nest) for _ in range(20)]  # All ants start at the nest
    ...
    while running:
        ...
        draw_counters(screen, ants, nest.total_food_collected, nest.total_ants_spawned, int(time_elapsed))
